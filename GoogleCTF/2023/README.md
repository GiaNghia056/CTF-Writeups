...
I use Trie to storage all words from given Dictionary and check if a character is suffix for a character. It helps me find the password. The following code is use to list all possible passwords.
```python
...
```
Try to decrypt the `secret` with all possible passwords.
```python
